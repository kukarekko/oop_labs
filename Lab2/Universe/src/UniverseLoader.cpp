...
Universe UniverseLoader::loadFromFile(const std::string& fileName) {
    ...
    if (!reader.readLine()) throw std::runtime_error("Empty file: " + fileName);
    ...
    while (reader.readLine()) {
        ...
        processLine(line, name, rule, cells, hasName, hasRule, duplicateCount,
                    invalidLineCount);
    }
    reader.close();
    ...
    std::cout << "Loaded: " << name << " (" << cells.size() << " cells)"
              << std::endl;
    return universe;
}
...
void UniverseLoader::processLine(const std::string& line, std::string& name,
    ...
    if (Parser::isNameLine(line)) {
        ...
        if (cells.find(coords) != cells.end()) {
            duplicateCount++;
            std::cout << "Warning: Duplicate coordinates: " << coords.first
        ...
        invalidLineCount++;
        std::cout << "Warning: Invalid line ignored: '" << line << "'"
                  << std::endl;
    }
}
